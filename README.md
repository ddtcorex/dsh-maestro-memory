...
pnpm test                       # full Vitest suite (10 files, 155 tests)
```
...
Source-grounded inventory (see `docs/plans/2026-08-24-dsh-maestro-memory-plan.md` capability table). Propose any as a separate plugin later.
...
- `docs/architecture.md` — seam table + disk schema + prompt contract.
...
```sh
pnpm install
pnpm test                       # full suite (10 files, 155 tests)
pnpm run verify                 # tsc --noEmit host + client (typecheck)
...
| Full | `pnpm test` | Entire suite (10 files, 155 tests) |
