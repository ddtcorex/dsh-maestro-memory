...
import { desensitize } from './memory/sanitize.ts'
...
function sanitize(text: string, doDesensitize: boolean): string | null {
  if (!doDesensitize) return text
 ...
 */
export function installAutoMemoryHooks(ctx: any, store: MaestroMemoryStore, opts: AutoMemoryOptions): () => void {
  if (!opts.enabled) return () => {}

  const disposers: Array<() => void> = []

  const onEvent = (_session: any, event: any) => {
    try {
      const cwd: string | undefined = _session?.header?.cwd ?? _session?.cwd ?? undefined
      if (event?.type === 'user/message' && opts.userMessage) {
        if (event?.data?.source?.kind !== undefined && event.data.source.kind !== 'user') return
        const text = extractText(event?.data?.content ?? [])
        if (!text) return
        const safe = sanitize(text, opts.desensitize)
        if (safe === null) return
        // Prefer project track when cwd present, else daily
        if (cwd) store.add('project', safe, cwd, { desensitize: false })
        else store.add('daily', safe, undefined, { desensitize: false })
