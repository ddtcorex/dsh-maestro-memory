...
/* iOS 16px field floor hold — this panel holds its fields at the same 16px.
   ...
   only reverts on blur. An earlier revision opted this panel's fields back out
   to the 13px it was designed at — and every tap on those fields zoomed the
   page on iPhone. Function beats pixels: hold 16px here (the 13px body scale
   elsewhere in the panel is untouched, as is every field on Android/desktop,
   which never carry the marker).
   ...
   !important rule would lose and the fields could silently shrink back. */
@media (max-width: 1023px) and (pointer: coarse) {
  ...
  html[data-mobile-nav-ios] .memx select:not(#dsh-memx-own-field-size) { font-size:16px !important; }
}
`
...
function HealthView({ ctx }: {ctx:any}): React.ReactElement {
  ...
  return React.createElement('div',{className:'memx-panel'},
    ...
    health.fiveDim?React.createElement('div',{className:'memx-card'},
      React.createElement('div',{className:'memx-dim-head'},
        React.createElement('span',{className:'memx-dim-title'}, '5-Dim Score'),
        React.createElement('span',{className:'memx-dim-formula'}, `composite ${Number(health.fiveDim.composite).toFixed(1)} = min×0.4+mean×0.6`),
      ),
      React.createElement('div',{className:'memx-dim-grid'},
        ...(['S','R','J','C','Safety'] as const).map(k=> React.createElement('div',{key:k, className:'memx-dim-card'}, React.createElement('div',{className:'memx-kicker'},k), React.createElement('div',{className:'memx-dim-val'}, String(health.fiveDim[k])))),
        React.createElement('div',{className:'memx-dim-card memx-dim-composite'}, React.createElement('div',{className:'memx-kicker'},'Composite'), React.createElement('div',{className:'memx-dim-val', style:{fontWeight:800}}, String(health.fiveDim.composite))),
      ),
    ):null,
...
function MemoryView({ ctx }: {ctx:any}): React.ReactElement {
  ...
  React.useEffect(()=>{
    const outer = document.querySelector('[data-conversation-scroll]') as HTMLElement | null
    if(outer) outer.scrollTop = 0
    const memxEl = document.querySelector('.memx') as HTMLElement | null
    if(memxEl) memxEl.scrollTop = 0
  },[])
