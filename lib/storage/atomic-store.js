 ...
 */
export function isDuplicate(entries, candidate) {
 ...
 * @returns index or -1
 */
export function findExactIndex(entries, exact) {
...
export async function isStaleLockAsync(lockPath) {
    try {
 ...
 */
export function withLockSync(dir, fn) {
    if (heldLocks.has(dir))
        return fn();
    ...
    for (;;) {
        ...
        if (Date.now() >= deadline)
            throw new Error('atomic-store: timed out waiting for directory lock');
...
export async function withLock(dir, fn) {
    ...
    for (;;) {
        ...
        if (acquired)
            break;
        if (await isStaleLockAsync(lockPath))
 ...
 */
export function createBackupSync(filePath) {
 ...
 */
export function readEntriesSync(filePath) {
    try {
 ...
 * @returns parsed entries
 */
export async function readEntries(filePath) {
    try {
 ...
 * @returns result
 */
export async function appendEntryAtomic(filePath, entry) {
    const dir = dirname(filePath);
 ...
 * @returns result
 */
export function appendEntryAtomicSync(filePath, entry) {
    const dir = dirname(filePath);
 ...
 * @returns result
 */
export async function writeEntriesAtomic(filePath, nextEntries) {
    const dir = dirname(filePath);
    return withLock(dir, async () => {
        let raw = '';
        try {
 ...
 * @returns result
 */
export function writeEntriesAtomicSync(filePath, nextEntries) {
    const dir = dirname(filePath);
    return withLockSync(dir, () => {
        let raw = '';
        try {
...
//# sourceMappingURL=atomic-store.js.map