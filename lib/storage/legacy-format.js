 ...
 * to this header.
 */
const ENTRY_HEAD_RE = /^(?:\[id:\s*[0-9a-f]{8}\]\s*)?(?:\[\d{4}-\d{2}-\d{2}(?: \d{1,2}:\d{2}(?::\d{2})?)?\]\s*|\[\d{1,2}:\d{2}(?::\d{2})?\]\s*)?(?:\[git [^\]]+\]\s*)*(?:\[branch:[^\]]*\]\s*)?(?:\[dsh-only\]\s*)?/i;
/**
 ...
 */
export function entryHeadPrefix(entry) {
...
export function stampTodoLine(meta, content) {
    ...
    if (meta.quadrant)
        parts.push(`[${meta.quadrant}]`);
    if (meta.due)
        parts.push(`[due: ${meta.due}]`);
    parts.push(`[status: ${meta.status ?? 'pending'}]`);
    if (meta.cat)
        parts.push(`[cat: ${meta.cat}]`);
    if (meta.doneAt)
        parts.push(`[done: ${meta.doneAt}]`);
    return `${parts.join(' ')}\n${String(content).trim()}`;
}
//# sourceMappingURL=legacy-format.js.map