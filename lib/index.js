...
import { resolveMemoryRoot, suggestionsPath, globalArchivePath, userArchivePath, projectKeyArchivePath, todoArchivePath, maestroMetaDir } from "./storage/layout.js";
...
export const inject = ['tools', 'systemPrompt', 'connection', 'sessions'];
            ...
            switch (endpoint) {
                ...
                case 'queue.decide': {
                    ...
                    if (action === 'approve') {
                        ...
                        let agent = payload?.cwd ? { session: { header: { cwd: String(payload.cwd) } } } : undefined;
                        if (!agent) {
                            try {
                                const snap = ctx.sessions?.list?.getSnapshot?.() ?? ctx.get?.('sessions')?.list?.getSnapshot?.();
        ...
        const proposeHandler = async (endpoint, payload) => {
            try {
                ...
                try {
                    const snap = ctx.sessions?.list?.getSnapshot?.() ?? ctx.get?.('sessions')?.list?.getSnapshot?.();
                    const cur = snap?.current;
                    sessionCwd = cur ? snap?.byId?.[cur]?.cwd ?? null : null;
                }
                catch { }
