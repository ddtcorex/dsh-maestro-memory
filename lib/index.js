...
import { listSkillsSync, listSkills, resolveDefaultMaestroSkillsDir } from "./skills-browser.js";
        ...
        const legacyHandler = async (endpoint, payload) => {
            switch (endpoint) {
                ...
                case 'memory.mutate': {
                    ...
                    try {
                        if (action === 'list') {
                            return { ok: true, entries: store.list(target, cwd, opts) };
                        }
                        if (action === 'add') {
                            return store.add(target, content ?? '', cwd, { branches: payload?.branches, summary: payload?.summary });
                        }
                        if (action === 'replace') {
                            return store.replace(target, match ?? '', content ?? '', cwd);
                        }
                        if (action === 'remove') {
                            return store.remove(target, match ?? '', cwd);
                        }
                        if (action === 'archive') {
                ...
                case 'skills.list': {
                    ...
                    try {
                        ...
                        if (roots) {
                            const entries = await listSkills({ roots: roots.map((r) => ({ dir: String(r.dir), origin: String(r.origin ?? 'custom') })) });
                            return { ok: true, entries };
                        }
