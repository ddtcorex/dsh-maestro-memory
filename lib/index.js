...
export function memoryTool(ctx, config, store, queue, getRuntime, archive) {
   ...
   * @returns {string} [Feedback] line text，或 ''。
   */
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      // target fallback validation: required relaxed to only ['action']（entries 批量模式
        ...
        switch (action) {
          case 'list': {
            ...
            if (protectedView) {
              ...
              message += `（this track has ${total}  entries, time span ${earliest || '?'} ~ ${latest || '?'}，defaults to latest 50 — to query earlier records add since/untilil（如 since=${earliest || 'YYYY-MM-DD'}）或增大 limit）`
          ...
          case 'add': {
            ...
            // 缺省/空 = all（无标记）。对不存在的分支只警告、照常写入（分支以后可能创建）。
          ...
          case 'expand': {
            ...
            // 只有当前分支可见的条目（无标记=all + [branch:…] 含当前分支）
