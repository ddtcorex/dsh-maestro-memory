        ...
        switch (action) {
          case 'list': {
            ...
            if (protectedView) {
              ...
              message += `（this track has ${total}  entries, time span ${earliest || '?'} ~ ${latest || '?'}，defaults to latest 50 — to query earlier records add since/untilil（如 since=${earliest || 'YYYY-MM-DD'}）or增大 limit）`
 ...
 *   且按 git 分支过滤（与 keyBranchFilter 一致：只注入无标记or覆盖当前
