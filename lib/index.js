...
export function renderSnapshot(config, store, agent, counter, sessionTitleService = null) {
  ...
  // aliases.json（用户手动设置，≤10 chars）。When both are absent, output is identical to the old version
...
${tail}${dueWarning}`)
  ...
  // changed to standalone message delivery (post-send in installBroadcast + first-session补投 unread
      ...
      try {
        switch (action) {
          case 'list': {
            ...
            // key 轨的 branch 过滤：只看该分支可见的条目（无标记=all + 标记含该分支）
