...
const COMPAT_TOOLS = ['memory', 'dtodo', 'skill_manage', 'memory_suggest', 'memory_review_status'];
/**
 ...
 */
export async function createFixtureProfile(opts) {
    const { profileDir, packageDir } = opts;
    if (!profileDir)
        throw new Error('createFixtureProfile: profileDir is required');
    if (!packageDir)
        throw new Error('createFixtureProfile: packageDir is required');
 ...
 */
export async function assertSingleOwner(profileDir, opts) {
    const owners = {};
    const errors = [];
    ...
    for (const tool of COMPAT_TOOLS) {
        const claimed = bundles.filter((b) => {
...
// ---------------------------------------------------------------------------
function writeLegacyMemory(file, entries) {
    const content = entries.length === 0 ? '' : entries.join('\n§\n') + '\n';
    mkdirSync(dirname(file), { recursive: true });
    writeFileSync(file, content, 'utf8');
}
function writeTodoFile(file, entriesRaw) {
 ...
 */
export async function createCopiedMemoryRoot(root, opts) {
