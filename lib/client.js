...
function HealthView({ ctx }) {
    ...
    const load = React.useCallback(async () => {
        ...
        try {
            ...
            const res = await conn.rpc.call('/maestro-memory/health', 'get', { cwd });
    ...
    return React.createElement('div', null, React.createElement('div', { style: { display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 } }, React.createElement('div', { className: 'card', style: { flex: 1, minWidth: 140 } }, React.createElement('div', { className: 'muted' }, 'Project entries'), React.createElement('div', { style: { fontWeight: 700 } }, String(health.project.total)), React.createElement('div', { className: 'muted' }, `${health.project.withSummary} with summary`)), React.createElement('div', { className: 'card', style: { flex: 1, minWidth: 140 } }, React.createElement('div', { className: 'muted' }, 'Coverage'), React.createElement('div', { style: { fontWeight: 700, color } }, `${cov.toFixed(1)}%`), React.createElement('div', { className: 'muted' }, cov >= 90 ? '✅ PASS' : '❌ FAIL (<90%)')), React.createElement('div', { className: 'card', style: { flex: 1, minWidth: 140 } }, React.createElement('div', { className: 'muted' }, 'Daily last 7d'), React.createElement('div', { style: { fontWeight: 600 } }, health.daily.counts.join(' · ')))), React.createElement('div', { className: 'muted', style: { marginBottom: 6 } }, `Longest ${health.longest.length} entries`), ...health.longest.map((it, i) => React.createElement('div', { key: i, className: 'card', style: { marginBottom: 6 } }, React.createElement('div', { className: 'muted' }, `${it.len} chars`), React.createElement('div', { style: { whiteSpace: 'pre-wrap' } }, it.preview))), React.createElement('button', { onClick: load, style: { marginTop: 12, padding: '6px 12px' }, 'data-testid': 'health-refresh' }, 'Refresh'));
}
