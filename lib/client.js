...
function MemoryListView({ ctx }) {
    ...
    return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 12 } }, React.createElement('div', { className: 'row' }, ['memory', 'user', 'key', 'project', 'daily'].map((k) => React.createElement('button', {
        key: k,
        onClick: () => setTrack(k),
        ...
        : null, React.createElement('button', { onClick: load, className: 'chip', 'data-testid': 'mem-refresh' }, 'Refresh')), msg ? React.createElement('div', { className: 'muted', style: { fontSize: 12 } }, msg) : null, loading
...
function MemoryView({ ctx }) {
    ...
    return React.createElement('div', { className: 'dshmem', style: { padding: 16, color: STYLE.text, display: 'flex', flexDirection: 'column' } }, React.createElement('style', null, MEM_CSS), React.createElement('div', { className: 'row', style: { marginBottom: 12 } }, ['memory', 'review', 'todos', 'skills'].map((k) => React.createElement('button', {
        key: k,
        onClick: () => setTab(k),
