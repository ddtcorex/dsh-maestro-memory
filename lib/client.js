...
/* When Memory is active, ensure its viewArea fully covers the transcript area and hide width handles (z-index 8) which would otherwise paint above it. Keep flex layout so composer remains visible below. */
[data-conversation-scroll]:has(.memx) { position: relative !important; overflow: hidden !important; }
[data-conversation-scroll]:has(.memx) [class*="viewArea"] { position: relative !important; z-index: 9 !important; background: var(--dsw-alias-bg-base) !important; isolation: isolate; contain: paint; display: flex !important; flex-direction: column !important; flex: 1 1 0 !important; min-height: 0 !important; height: auto !important; width: 100% !important; max-width: 100% !important; overflow: hidden !important; pointer-events: auto !important; }
[data-conversation-scroll]:has(.memx) [class*="viewArea"] > [data-slot="conversation.view"] { display: flex !important; flex-direction: column !important; flex: 1 1 auto !important; min-height: 0 !important; min-width: 0 !important; width: 100% !important; max-width: 100% !important; overflow: hidden !important; background: var(--dsw-alias-bg-base) !important; pointer-events: auto !important; }
[data-conversation-scroll]:has(.memx) .memx { flex: 1 1 auto !important; min-height: 0 !important; overflow-y: auto !important; pointer-events: auto !important; overscroll-behavior: contain !important; }
[class*="body"]:has(.memx) [data-width-handle], [data-conversation-scroll]:has(.memx) ~ [data-width-handle], body:has(.memx) [data-width-handle], [class*="root"]:has(.memx) [data-width-handle] { display: none !important; pointer-events: none !important; }
...
.memx-dim-grid { display:grid; gap:8px; width:100%; max-width:100%; min-width:0; grid-template-columns: repeat(2, minmax(0,1fr)); }
@media (min-width: 640px) { .memx-dim-grid { grid-template-columns: repeat(3, minmax(0,1fr)); } }
@media (min-width: 1024px) { .memx-dim-grid { grid-template-columns: repeat(6, minmax(0,1fr)); } }
.memx-dim-card { border:1px solid var(--dsw-alias-border-l1); border-radius:10px; background:var(--dsw-alias-bg-layer-2); padding:10px 8px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:4px; min-height:64px; text-align:center; min-width:0; }
.memx-dim-card .memx-kicker { font-size:11px; }
.memx-dim-val { font-size:18px; font-weight:700; line-height:1; }
.memx-dim-composite { background:var(--dsw-alias-interactive-bg-active); border-color:var(--dsw-alias-border-l2); }
@media (max-width: 639px) { .memx-dim-composite { grid-column: span 2; } }
@media (min-width: 640px) and (max-width: 1023px) { .memx-dim-composite { grid-column: span 3; } }
.memx-dim-head { display:flex; flex-wrap:wrap; align-items:baseline; gap:4px 8px; margin-bottom:8px; }
.memx-dim-title { font-weight:600; color:var(--dsw-alias-label-primary); font-size:13px; }
.memx-dim-formula { font-size:11px; color:var(--dsw-alias-label-secondary); }
@media (max-width: 480px) { .memx-dim-head { flex-direction:column; align-items:flex-start; gap:2px; } }
    ...
    const decide = React.useCallback(async (action, index) => {
        ...
        if (action === 'approve') {
            try {
                ...
                const cwd = cur ? snap?.byId?.[cur]?.cwd ?? '' : '';
                if (cwd)
                    payload.cwd = cwd;
            }
            catch { }
...
function HealthView({ ctx }) {
    ...
    if (!health)
        return React.createElement('div', null, msg ? React.createElement('div', { style: { color: 'var(--dsw-alias-state-error-primary)', marginBottom: 8 } }, msg) : null, React.createElement('div', { className: 'memx-muted' }, 'No data'), React.createElement('button', { onClick: load, className: 'memx-btn', style: { marginTop: 8 } }, 'Retry'));
    ...
    return React.createElement('div', { className: 'memx-panel' }, React.createElement('div', { className: 'memx-grid memx-grid-4' }, React.createElement('div', { className: 'memx-card memx-stat' }, React.createElement('div', { className: 'memx-kicker' }, 'Project entries'), React.createElement('div', { className: 'memx-statval' }, String(health.project.total)), React.createElement('div', { className: 'memx-sm' }, `${health.project.withSummary} with summary`)), React.createElement('div', { className: 'memx-card memx-stat' }, React.createElement('div', { className: 'memx-kicker' }, 'Coverage'), React.createElement('div', { className: 'memx-statval', style: { color: covColor } }, `${cov.toFixed(1)}%`), React.createElement('div', { className: 'memx-sm' }, cov >= 90 ? 'PASS' : 'FAIL (<90%)')), React.createElement('div', { className: 'memx-card memx-stat' }, React.createElement('div', { className: 'memx-kicker' }, 'Daily last 7d'), React.createElement('div', { style: { fontWeight: 600 } }, health.daily.counts.join(' · ')), React.createElement('div', { className: 'memx-sm' }, 'entries per day')), React.createElement('div', { className: 'memx-card memx-stat' }, React.createElement('div', { className: 'memx-kicker' }, 'Discipline'), React.createElement('div', { style: { fontWeight: 600 } }, '1.5 avg / session'), React.createElement('div', { className: 'memx-sm' }, 'target 1+'))), health.fiveDim ? React.createElement('div', { className: 'memx-card' }, React.createElement('div', { className: 'memx-dim-head' }, React.createElement('span', { className: 'memx-dim-title' }, '5-Dim Score'), React.createElement('span', { className: 'memx-dim-formula' }, `composite ${Number(health.fiveDim.composite).toFixed(1)} = min×0.4+mean×0.6`)), React.createElement('div', { className: 'memx-dim-grid' }, ...['S', 'R', 'J', 'C', 'Safety'].map(k => React.createElement('div', { key: k, className: 'memx-dim-card' }, React.createElement('div', { className: 'memx-kicker' }, k), React.createElement('div', { className: 'memx-dim-val' }, String(health.fiveDim[k])))), React.createElement('div', { className: 'memx-dim-card memx-dim-composite' }, React.createElement('div', { className: 'memx-kicker' }, 'Composite'), React.createElement('div', { className: 'memx-dim-val', style: { fontWeight: 800 } }, String(health.fiveDim.composite))))) : null, React.createElement('div', { className: 'memx-muted' }, `Longest ${health.longest.length} entries`), React.createElement('div', { className: 'memx-grid' }, ...health.longest.map((it, i) => React.createElement('div', { key: i, className: 'memx-card', style: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 } }, React.createElement('div', { style: { flex: 1, minWidth: 0 } }, React.createElement('div', { className: 'memx-xs' }, `${it.len} chars`), React.createElement('div', { className: 'memx-pre-sm' }, it.preview)), React.createElement('button', { onClick: async () => {
            try {
                let cwd = '';
                try {
                    const snap = ctx?.sessions?.list?.getSnapshot?.();
                    const cur = snap?.current;
                    cwd = cur ? snap?.byId?.[cur]?.cwd ?? '' : '';
                }
                catch { }
                const res = await rpc('queue.propose', { content: it.preview, reason: 'promote from Health longest', cwd });
                setMsg(res?.queued ? `proposed (queue ${res.queued})` : `proposed: ${JSON.stringify(res).slice(0, 80)}`);
            }
            catch (e) {
...
function MemoryView({ ctx }) {
    ...
    // Hide width-handle glow while Memory overlay is active (JS fallback for :has, and ensures pointer cover even if CSS :has is slow).
    React.useEffect(() => {
        const handles = Array.from(document.querySelectorAll('[data-width-handle]'));
        const prev = handles.map(h => h.style.display);
        handles.forEach(h => { h.style.display = 'none'; });
        return () => { handles.forEach((h, i) => { h.style.display = prev[i] ?? ''; }); };
    }, []);
    // Reset scroll positions when Memory view mounts (outer tab selected) — prevents outer scroll from chat carrying over and clipping header.
    React.useEffect(() => {
        const outer = document.querySelector('[data-conversation-scroll]');
        if (outer)
            outer.scrollTop = 0;
        const memxEl = document.querySelector('.memx');
        if (memxEl)
            memxEl.scrollTop = 0;
    }, []);
    // Keep memx at top when switching inner tabs; prevents auto scroll-down after content swap.
    React.useEffect(() => {
        const memxEl = document.querySelector('.memx');
        if (memxEl)
            memxEl.scrollTop = 0;
