...
exports.inject = ['slots', 'locale', 'conversation', 'sessions', 'connection'];
...
function ReviewQueueView({ ctx }) {
    ...
    if (entries.length === 0) {
        return React.createElement('div', null, React.createElement('div', { style: { opacity: 0.7, marginBottom: 8 } }, 'No pending suggestions'), msg ? React.createElement('div', { style: { fontSize: 12, color: '#666' } }, msg) : null, React.createElement('button', { onClick: load, style: { marginTop: 8 } }, 'Refresh'));
    }
    ...
    }), msg ? React.createElement('div', { style: { marginTop: 8, fontSize: 12, color: '#333' } }, msg) : null, React.createElement('button', { onClick: load, style: { marginTop: 8 } }, 'Refresh'));
}
function TodosView({ ctx }) {
    ...
    return React.createElement('div', null, React.createElement('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 } }, ['all', 'life', 'work', 'project', 'daily'].map((k) => React.createElement('button', {
    ...
    }), 'expired'), React.createElement('button', { onClick: load, style: { padding: '4px 8px', fontSize: 12 }, 'data-testid': 'todo-refresh' }, 'Refresh')), React.createElement('div', { style: { border: '1px solid #eee', borderRadius: 8, padding: 8, marginBottom: 12 } }, React.createElement('div', { style: { fontWeight: 600, marginBottom: 6, fontSize: 13 } }, 'Add todo'), React.createElement('textarea', {
        value: draftContent,
    ...
    }, 'Add')), msg ? React.createElement('div', { style: { fontSize: 12, color: '#333', marginBottom: 8, whiteSpace: 'pre-wrap' } }, msg) : null, loading
        ...
        : items.length === 0
            ...
            : React.createElement('div', null, React.createElement('div', { style: { fontSize: 12, opacity: 0.7, marginBottom: 6 } }, `${items.length} todos${!showAll && items.length === 8 ? ' (smart view limited to 8)' : ''}`), ...items.map((it) => React.createElement('div', { key: it.id, style: { border: '1px solid #ddd', borderRadius: 8, padding: 8, marginBottom: 8 } }, React.createElement('div', { style: { fontSize: 12, opacity: 0.7 } }, `[${it.target}]`, it.quadrant ? ` [${it.quadrant}]` : '', it.due ? ` [due:${it.due}]` : '', it.status !== 'pending' ? ` [${it.status}]` : '', it.cat ? ` [cat:${it.cat}]` : '', it.doneAt ? ` [done:${it.doneAt}]` : '', it.past ? ` [past ${it.day}]` : '', ` id:${it.id}`), editId === it.id
...
function SkillsView({ ctx }) {
    ...
    return React.createElement('div', null, React.createElement('div', { style: { fontSize: 12, opacity: 0.7, marginBottom: 8 } }, `${entries.length} skills (read-only browser, metadata/origin only — maestro-skills discovery unchanged)`), entries.length === 0
        ...
        : React.createElement('div', null, ...entries.map((e) => React.createElement('div', { key: e.name, style: { border: '1px solid #ddd', borderRadius: 8, padding: 8, marginBottom: 8 } }, React.createElement('div', { style: { fontWeight: 600 } }, e.name), React.createElement('div', { style: { fontSize: 12, opacity: 0.7 } }, `[${e.origin}] ${e.path}`), React.createElement('div', { style: { margin: '4px 0', fontSize: 13, whiteSpace: 'pre-wrap' } }, e.description), e.metadata && Object.keys(e.metadata).length > 0
            ? React.createElement('div', { style: { fontSize: 11, opacity: 0.6, marginTop: 4 } }, `metadata: ${Object.entries(e.metadata)
                .map(([k, v]) => `${k}=${String(v).slice(0, 60)}`)
                .join(', ')}`)
            : null))), msg ? React.createElement('div', { style: { marginTop: 8, fontSize: 12, color: '#333' } }, msg) : null, React.createElement('button', { onClick: load, style: { marginTop: 8, padding: '4px 8px', fontSize: 12 }, 'data-testid': 'skills-refresh' }, 'Refresh'), React.createElement('div', { style: { marginTop: 8, fontSize: 11, opacity: 0.6 } }, 'Read-only — model suggestions cannot change skills. Future edits, if approved, will require explicit user action + path containment.'));
}
function MemoryView({ ctx }) {
    const [tab, setTab] = React.useState('review');
    return React.createElement('div', { style: { padding: 16, fontFamily: 'system-ui, sans-serif' } }, React.createElement('div', { style: { display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' } }, ['memory', 'review', 'todos', 'skills'].map((k) => React.createElement('button', {
        key: k,
        onClick: () => setTab(k),
        style: {
