/**
 * maestro-memory — todo store and `dtodo` tool (four tracks) (legacy: dsh-memory-evolve).
 *
 ...
 *   Take mom to the hospital for follow-up
 *   Remember to bring the test report
 *
 ...
 *
 * @module maestro-memory/todo (legacy: dsh-memory-evolve/todo)
 */
...
/** Timestamp on the first line of an entry (creation time, auto-stamped by the program). */
const TIME_RE = /^\[(\d{4}-\d{2}-\d{2}(?:\s\d{2}:\d{2})?)\]\s*/
/** Unique id (8 hex chars). */
const ID_RE = /\[id:\s*([0-9a-f]{8})\]/i
/** Quadrant: q1 important+urgent / q2 important+not urgent / q3 urgent+not important / q4 not important+not urgent. */
const QUAD_RE = /\[q([1-4])\]/i
/** Due date. */
const DUE_RE = /\[due:\s*(\d{4}-\d{2}-\d{2})\]/i
/** Status. */
const STATUS_RE = /\[status:\s*(pending|doing|done|blocked|cancelled)\]/i
/** Completion time (auto-stamped by the program). */
const DONE_RE = /\[done:\s*(\d{4}-\d{2}-\d{2}(?:\s\d{2}:\d{2})?)\]/i
/** Category. */
const CAT_RE = /\[cat:\s*([^\]]+)\]/i

/** Current timestamp `YYYY-MM-DD HH:MM` (local time). */
function nowStamp() {
  const d = new Date()
...
export class TodoStore {
   ...
   * @param {(cwd: string) => string} [projectDirResolver] - project directory resolver
   *   (sync-initialized projects -> projectId directory; projects without sync -> projectHash directory).
   *   Same injection as MemoryStore: after memory-sync migration, project todos must follow
   *   the new directory, otherwise TODOS.md would land in the old directory and never enter
   *   the sync repo (2026-08-11 unified model).
   */
  ...
  fileOf(target, cwd, date) {
    switch (target) {
      ...
      case 'project': {
        if (!cwd) throw new Error('maestro-memory: project todo requires a session working directory')
        // Resolver injected (memory-sync assembly) -> project todos follow the memory directory (projectId);
        // otherwise keep the old path (projectHash) — no change for projects without sync
      ...
      default:
        throw new Error(`maestro-memory: invalid todo track "${target}"`)
    }
  }
  ...
  parseAll(text) {
    // Strip the file-header comment block, then also remove any leading `§` delimiter left behind
    const body = text
      .replace(/^<!--[\s\S]*?-->\s*/, '')
      .replace(/^\s*§\s*\n?/, '')
      .trim()
  ...
  itemsOf(target, cwd, date) {
    ...
    // Daily entries carry their owning date (today or the specified date) for past queries / write-back file location
    if (target === 'daily') {
  ...
  listTodos(targets, options = {}, cwd, today = todayStamp()) {
    ...
    // "Expired" for past daily entries: unfinished and (when no due, by file date) already past today
    ...
    // Past daily todos: all daily files before today; filter "expired" leftovers by default
    ...
    // Normal entries: overdue > today > q1 > q2 > q3 > q4 > none; past entries are uniformly last
    // (by date descending, intra-group time descending). Then stably sorted by due then time.
    ...
    // Decision (2026-08-09): project todo list (single-track project query) is displayed reversed —
    // most recently added entries appear first (original order was due/time ascending, old entries first).
    // Only applies to single-track project: mixed queries (default four tracks) and other tracks keep
    // the original sorting; side effect: overdue sinks to the bottom after reversal (the trailing
    // dtodo list check still reminds about dues).
    if (targets.length === 1 && targets[0] === 'project') {
      selected.reverse()
    }
    // past=true is an explicit history query: no truncation, same as all=true
    const explicit = options.all === true || options.past === true
    const truncated = !explicit && selected.length > DEFAULT_VIEW_LIMIT
    // Defensive hint: querying past without expired — daily todos are due on the same day, unfinished past
    // entries are hidden by default and the model might think "no todos yesterday"; the output directly
    // points out the remedial parameter.
  ...
  formatList(result, today = todayStamp()) {
    ...
    if (items.length === 0) {
      ...
      return defaultView
        ? `Todos (default view): no unfinished todos needing attention (overdue / due today / current project / important & urgent) — all clear 🎉${hint}`
        : `Todos: no matching entries${hint}`
    }
    ...
    const lines = items.map((item) => {
      ...
      if (item.past === true) tags.push(`[past ${item.day}]`)
      if (item.quadrant) tags.push(`[${item.quadrant}]`)
      if (item.due !== null) tags.push(item.due < today ? `[overdue ${item.due}]` : `[${item.due}]`)
    ...
    const hint = result.hint ? `\n${result.hint}` : ''
    return `${head}\n${lines.join('\n')}\ntag semantics: q1-q4 = quadrants (important x urgent); due = deadline; status = state; operate by id (dtodo done/update/remove <id>, may include target=).${hint}`
  }
}
 ...
 * @param {() => boolean} [isTodoEnabled] - reads the live todo-capability
 * setting. Tool definitions may outlive a settings change, so execution
 * checks the setting before handling a request.
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        status: { type: 'string', enum: TODO_STATUSES, get description() { return tt('todo.status') } },
    ...
    output: {
      schema: {
        ...
        properties: {
          ok: { type: 'boolean' },
          code: { type: 'string', description: 'Machine-readable failure code; TODO_DISABLED when todo is disabled.' },
          message: { type: 'string' },
          target: { type: 'string' },
          id: { type: 'string', description: 'on successful add, returns the new entry id (for subsequent done/update/remove)' },
        },
        required: ['ok', 'message'],
      },
    ...
    async execute(args, exec) {
      ...
      if (action === 'list') {
        ...
        // Cross-project query: cwd parameter explicitly specifies the project working directory (default = current session directory)
...
/**
 * Registers the dtodo tool while the live todo setting is enabled.
 * @param {object} ctx - plugin context with the tools service.
 * @param {object} config - resolved plugin configuration.
 * @param {() => object} getRuntime - reads the live runtime configuration.
 * @param {TodoStore} todoStore - persistent todo storage.
 * @returns {{sync: () => void, enabled: () => boolean}} lifecycle controller.
 */
