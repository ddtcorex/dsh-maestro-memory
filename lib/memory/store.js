...
import { serializeEntries, readEntriesSync, appendEntryAtomicSync, writeEntriesAtomicSync, writeAtomicSync, withLockSync, } from "../storage/atomic-store.js";
...
export class MaestroMemoryStore {
    ...
    repairKeyDelimiter(cwd) {
        ...
        return withLockSync(dirname(file), () => {
            ...
            const entries = raw
                ...
                .filter(e => e.length > 0);
            if (entries.length <= 1)
                return { ok: true, repaired: 0 }; // already canonical or empty/single
    ...
    pruneGlobalMemory() {
        ...
        return withLockSync(dirname(file), () => {
            ...
            if (fitted.length === entries.length)
                return { ok: true, removed: 0 };
...
function fitSection(entries, cap) {
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (used + size > cap && keptDesc.length > 0)
            break;
